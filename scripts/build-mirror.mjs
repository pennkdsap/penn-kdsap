...
const pathwayItems = homeContent.pathways.items.map((item) => `
            <article class="path-card">
              ...
              <a href="${escapeHtml(item.actionUrl)}"${item.actionLabel === homeContent.headerAction.label ? ' data-analytics-event="screening_request_clicked" data-analytics-location="pathway_card"' : ''}>${escapeHtml(item.actionLabel)} <span aria-hidden="true">→</span></a>
            </article>`).join('');
  ...
  if (name === 'screenings') return `
      ...
      <section class="interior-section section-tint" data-reveal>
        <div class="shell">
          ...
          <a class="button button-dark section-action" href="${escapeHtml(data.actionUrl)}" data-analytics-event="screening_request_clicked" data-analytics-location="screening_page">${escapeHtml(data.actionLabel)}</a>
        </div>
      </section>`;
